  ...
  rules: {
    ...
    switch_expression: ($) =>
      seq(
        ...
        field('body', seq('{', repeat($.switch_case), optional($.switch_else), '}')),
      ),
    ...
    enum_member: ($) => seq($.identifier, optional(seq('=', field('value', $._enum_value)))),

    enum_declaration: ($) =>
      seq('enum', $.identifier, '{', commaSep1($.enum_member), optional(','), '}'),
    ...
    _assignment_identifier: ($) => choice($.identifier, $.member_expression),

    _enum_value: ($) => choice($.number, alias($._number, $.unary_expression), $.string),
    ...
    arguments: ($) => seq('(', commaSep(optional(choice($.expression, $.spread_argument))), ')'),
    ...
    variable_assignment: ($) =>
      seq(field('assignee', $._assignment_identifier), '=', field('value', $.expression), ';'),
    ...
    _declaration_kind: ($) => choice($.const, $.let),
    ...
    object_literal: ($) =>
      seq(
        '{',
        commaSep1(choice($.key_value_pair, alias($.identifier, $.shorthand_property_identifier))),
        optional(','),
        '}',
      ),
    ...
    key_value_pair: ($) => seq($.identifier, ':', $.expression),
